 ...
 * and Dispatch the response to respective {@link EzDownloadRequest}
 * EzDownloadDispatcher
 * Created by renwoxing on 2018/2/7.
 */
class EzDownloadDispatcher extends Thread {

    private static final String TAG = EzDownloadDispatcher.class.getSimpleName();

    /**
     * The queue of download requests to service.
     */
    private final BlockingQueue<EzDownloadRequest> mQueue;
    /**
     ...
     */
    private EzDownloadRequestQueue.CallBackDelivery mDelivery;
    /**
     ...
     */
    EzDownloadDispatcher(BlockingQueue<EzDownloadRequest> queue,
                         EzDownloadRequestQueue.CallBackDelivery delivery) {
        mQueue = queue;
        mDelivery = delivery;
    }
    ...
    public void run() {
        ...
        mTimer = new Timer();
        while (true) {
            EzDownloadRequest request = null;
            try {
                request = mQueue.take();
                mRedirectionCount = 0;
                shouldAllowRedirects = true;
                Log.d(TAG, "Download initiated for " + request.getDownloadId());
                updateDownloadState(request, EzDownloadStatus.STATUS_STARTED);
                ...
                if (mQuit) {
                    if (request != null) {
                        ...
                        if (request.getDownloadState() != EzDownloadStatus.STATUS_SUCCESSFUL) {
                            updateDownloadFailed(request, EzDownloadStatus.ERROR_DOWNLOAD_CANCELLED, "Download cancelled");
                        }
                    }
                    mTimer.cancel();
                    return;
    ...
    private void executeDownload(EzDownloadRequest request, String downloadUrl) {
        URL url;
        try {
            url = new URL(downloadUrl);
        } catch (MalformedURLException e) {
            updateDownloadFailed(request, EzDownloadStatus.ERROR_MALFORMED_URI, "MalformedURLException: URI passed is malformed.");
            return;
        }
        ...
        try {
            ...
            updateDownloadState(request, EzDownloadStatus.STATUS_CONNECTING);

            final int responseCode = conn.getResponseCode();

            Log.d(TAG, "Response code obtained for downloaded Id "
            ...
            switch (responseCode) {
                ...
                case HTTP_OK:
                    ...
                    if (readResponseHeaders(request, conn, responseCode) == 1) {
                        ...
                        updateDownloadFailed(request, EzDownloadStatus.ERROR_DOWNLOAD_SIZE_UNKNOWN, "Transfer-Encoding not found as well as can't know size of download, giving up");
                    }
                    return;
                ...
                case HTTP_TEMP_REDIRECT:
                    ...
                    if (mRedirectionCount > MAX_REDIRECTS && shouldAllowRedirects) {
                        updateDownloadFailed(request, EzDownloadStatus.ERROR_TOO_MANY_REDIRECTS, "Too many redirects, giving up");
                        return;
                    }
                    break;
                ...
                case HTTP_INTERNAL_ERROR:
                    ...
                    break;
                default:
                    updateDownloadFailed(request, EzDownloadStatus.ERROR_UNHANDLED_HTTP_CODE, "Unhandled HTTP response:" + responseCode + " message:" + conn.getResponseMessage());
                    break;
            }
            ...
            e.printStackTrace();
            updateDownloadFailed(request, EzDownloadStatus.ERROR_HTTP_DATA_ERROR, "Trouble with low-level sockets");
        } finally {
            if (conn != null) {
                conn.disconnect();
            }
    ...
    private void transferData(EzDownloadRequest request, HttpURLConnection conn) {
            ...
            if (!destinationFile.exists()) {
                try {
                    ...
                    if (!destinationFile.createNewFile()) {
                        ...
                        updateDownloadFailed(request, EzDownloadStatus.ERROR_FILE_ERROR,
    ...
    private void transferData(EzDownloadRequest request, InputStream in, RandomAccessFile out) {
        final byte data[] = new byte[BUFFER_SIZE];
        long mCurrentBytes = mDownloadedCacheSize;
        request.setDownloadState(EzDownloadStatus.STATUS_RUNNING);
        Log.d(TAG, "Content Length: " + mContentLength + " for Download Id " + request.getDownloadId());
        for (; ; ) {
            if (request.isCancelled()) {
                Log.d(TAG, "Stopping the download as Download Request is cancelled for Downloaded Id " + request.getDownloadId());
                request.finish();
                updateDownloadFailed(request, EzDownloadStatus.ERROR_DOWNLOAD_CANCELLED, "Download cancelled");
                return;
            }
            ...
            if (writeDataToDestination(request, data, bytesRead, out)) {
                ...
                request.finish();
                updateDownloadFailed(request, EzDownloadStatus.ERROR_FILE_ERROR, "Failed writing file");
                return;
            }
        }
    }

    private int readFromResponse(EzDownloadRequest request, byte[] data, InputStream entityStream) {
        try {
            ...
            updateDownloadFailed(request, EzDownloadStatus.ERROR_HTTP_DATA_ERROR, "IOException: Failed reading response");
            return Integer.MIN_VALUE;
        }
    }

    private boolean writeDataToDestination(EzDownloadRequest request, byte[] data, int bytesRead, RandomAccessFile out) {
        boolean successInWritingToDestination = true;
        try {
            out.write(data, 0, bytesRead);
        } catch (IOException ex) {
            updateDownloadFailed(request, EzDownloadStatus.ERROR_FILE_ERROR, "IOException when writing download contents to the destination file");
            successInWritingToDestination = false;
        } catch (Exception e) {
            updateDownloadFailed(request, EzDownloadStatus.ERROR_FILE_ERROR, "Exception when writing download contents to the destination file");
            successInWritingToDestination = false;
        }
    ...
    private int readResponseHeaders(EzDownloadRequest request, HttpURLConnection conn, int responseCode) {
        ...
        if (transferEncoding == null) {
            ...
            Log.d(TAG, "Ignoring Content-Length since Transfer-Encoding is also defined for Downloaded Id " + request.getDownloadId());
        }
    ...
    private void attemptRetryOnTimeOutException(final EzDownloadRequest request) {
        updateDownloadState(request, EzDownloadStatus.STATUS_RETRYING);
        final IEzDownloadRetryPolicy retryPolicy = request.getRetryPolicy();
        try {
            retryPolicy.retry();
        ...
        } catch (EzDownloadRetryErrorException e) {
            // Update download failed.
            updateDownloadFailed(request, EzDownloadStatus.ERROR_CONNECTION_TIMEOUT_AFTER_RETRIES,
     ...
     */
    private void cleanupDestination(EzDownloadRequest request, boolean forceClean) {
    ...
    private void updateDownloadState(EzDownloadRequest request, int state) {
        request.setDownloadState(state);
    }

    private void updateDownloadComplete(EzDownloadRequest request) {
        mDownloadedCacheSize = 0; // reset into Zero.
        mDelivery.postDownloadComplete(request);
        request.setDownloadState(EzDownloadStatus.STATUS_SUCCESSFUL);
        request.finish();
    }

    private void updateDownloadFailed(EzDownloadRequest request, int errorCode, String errorMsg) {
        mDownloadedCacheSize = 0; // reset into Zero.
        shouldAllowRedirects = false;
        request.setDownloadState(EzDownloadStatus.STATUS_FAILED);
    ...
    private void updateDownloadProgress(EzDownloadRequest request, int progress, long downloadedBytes) {
