...
/**
 * 下载队列
 * EzDownloadRequestQueue
 * Created by renwoxing on 2018/2/7.
 */
public class EzDownloadRequestQueue {
     ...
     */
    private Set<EzDownloadRequest> mCurrentRequests = new HashSet<>();

    /** The queue of requests that are actually going out to the network. */
    private PriorityBlockingQueue<EzDownloadRequest> mDownloadQueue = new PriorityBlockingQueue<>();

    /** The download dispatchers */
    private EzDownloadDispatcher[] mDownloadDispatchers;
     ...
     * Deprecated:
     */
    public EzDownloadRequestQueue(int threadPoolSize) {
     ...
     */
    public EzDownloadRequestQueue(Handler callbackHandler) throws InvalidParameterException {
    ...
    public void start() {
        ...
        for (int i = 0; i < mDownloadDispatchers.length; i++) {
            EzDownloadDispatcher downloadDispatcher = new EzDownloadDispatcher(mDownloadQueue, mDelivery);
    ...
    int query(int downloadId) {
        synchronized (mCurrentRequests) {
            for (EzDownloadRequest request : mCurrentRequests) {
                if (request.getDownloadId() == downloadId) {
                    return request.getDownloadState();
                }
            }
        }
        return EzDownloadStatus.STATUS_NOT_FOUND;
    }
    ...
    /**
     * This is called by methods that want to throw an exception if the {@link EzDownloadRequest}
    ...
    void finish(EzDownloadRequest request) {
    ...
    private void initialize(Handler callbackHandler) {
        ...
        mDownloadDispatchers = new EzDownloadDispatcher[processors];
    ...
    private void initialize(Handler callbackHandler, int threadPoolSize) {
        mDownloadDispatchers = new EzDownloadDispatcher[threadPoolSize];
        mDelivery = new CallBackDelivery(callbackHandler);
    }
